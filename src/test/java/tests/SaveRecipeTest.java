...
//       TC_01 - OPEN SHARE DIALOG
    ...
    public void RecipeDelete_TC_01() {
        ...
        // ✅ verify tồn tại
        Assert.assertTrue(
                saved.isRecipeExist(title, ""),
                "❌ Không tìm thấy công thức đã lưu"
        );

        // 🔥 verify nằm top (quan trọng)
        String topTitle = saved.getTitleAt(0);
        Assert.assertTrue(
                topTitle.contains(title),
                "❌ Item mới không nằm ở đầu list"
        );

        int before = saved.getTotalRecipes();
        ...
        int after = saved.getTotalRecipes();
    ...
    // TC_02 – CANCEL DELETE RECIPE
    @Test(priority = 20,
            description = "RecipeDelete_TC_02 - Hủy xóa công thức")
    public void RecipeDelete_TC_02() {
        ...
        Assert.assertEquals(
                after,
                before,
                "❌ Cancel nhưng vẫn bị xóa"
        );

        // 🔥 verify item vẫn tồn tại
        Assert.assertTrue(
                saved.isRecipeExist(title, ""),
                "❌ Công thức bị mất sau khi cancel"
        );
    }

    // TC_03 – DELETE RECIPE KHI CHƯA LOGIN
    @Test(priority = 21)

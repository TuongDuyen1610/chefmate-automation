...
public class SearchTest extends BaseTest {
    ...
    /**
     * Normalize giống SearchFlow để match có dấu/không dấu (vd "trung ga" ~ "Trứng gà")
     */
...
////    =========TC_01 DEMO MORE CLEAR=========
//    @Test(priority = 1, description = "RecipeName_TC_01 - Hiển thị danh sách kết quả đầy đủ (demo scroll chậm)")
...
//    @Test(priority = 2, description = "RecipeName_TC_02 - Không phân biệt hoa/thường, có dấu/không dấu")
//    @Feature("TÍNH NĂNG 2: Tìm kiếm theo Tên món / Nguyên liệu")
//    @Story("Happy Case")
...
//
//            // ✅ CHỈ verify khi có result
//            if (searchScreen.getResultCount() > 0) {
//                verifyIngredientKeywordSmart(k);
//            }
//
////            // ✅ CHỤP ẢNH NGAY KHI UI CÒN SỐNG
////            AllureHelper.attachScreenshot("Search - " + k);
//
//            // ✅ QUAN TRỌNG NHẤT: RESET UI
//            searchScreen.clickBack();
//
//            // ✅ THÊM NGHỈ NHẸ (TRÁNH CRASH)
//            WaitingHelper.sleepSeconds(2);
//        }
//    }
//
...
//    }
    ...
    public void RecipeTag_TC_01() {
        ...
        if (hasResult) {
            // 👉 vào detail + scroll + match (chị đã fix ở screen rồi)
            verifyTagKeywordInDetail(keyword);
        } else {
            // 👉 vẫn cần evidence
            AllureHelper.attachScreenshot("NO RESULT TAG - " + keyword);
...
//    @Test(priority = 12, description = "RecipeTag_TC_02 - Tag hiển thị label/badge (verify tags section in detail)")

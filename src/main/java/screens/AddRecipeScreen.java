...
public class AddRecipeScreen extends BaseScreen {
    private static final Logger logger = LoggerFactory.getLogger(AddRecipeScreen.class);

    // ===== HEADER =====
    private final By header = By.xpath("//android.widget.TextView[@text='Thêm công thức']");
    ...
    private final By imageEmulator_1 =
            By.xpath("(//android.widget.ImageView[@resource-id='com.google.android.documentsui:id/icon_thumb'])[1]");
    ...
    // ✅ Nút "Thêm" ngoài màn Add: scope theo section Tags để tránh click nhầm
    ...
    // ===== INGREDIENT =====
    private By ingredientField(int row, int col) {
        return By.xpath("//android.widget.ScrollView/android.view.View[" + row + "]/android.widget.EditText[" + col + "]");
    }
    ...
    // ===== STEP =====
    private By stepField(int index) {
        return By.xpath("//android.widget.ScrollView/android.widget.EditText[" + index + "]");
    }
    ...
    private final By btnPost = By.xpath("//android.widget.TextView[@text='Đăng công thức']");
    ...
    //Vào đến Màn home, app click vào thanh ngang 3 sọc
    private  final By tap3soc = By.xpath("//android.view.View[@content-desc='Options']");
...
//    private  final By gallery2 = By.xpath("(//android.widget.ImageView[@resource-id='android:id/icon'])[3]");
    ...
    private void scrollToElementCenter(By locator) {
        // Scroll nhẹ cho tới khi element hiện, sau đó đưa nó về khoảng giữa màn hình bằng 1 swipe nhỏ
        for (int i = 0; i < 6; i++) {
    ...
    private void typeAndHideKeyboard(By locator, String text) {
        scrollToElementCenter(locator);
        WaitingHelper.sleepSeconds(1); // ✅ cực quan trọng để UI settle

        // refind mỗi lần để tránh stale
        WebElement el = getDriver().findElement(locator);
        el.click();
        try { el.clear(); } catch (Exception ignored) {}
        el.sendKeys(text);

        // ✅ hide keyboard ngay sau khi nhập xong ô đó
    ...
    private void ensureVisible(By locator) {
        ...
        while (!isDisplayed(locator) && retry < 4) {
            slowSwipeDownOnScreen(1);
            WaitingHelper.sleepSeconds(1);
            retry++;
        }
    }
    private void waitAfterAddIngredient() {
        // Chờ UI settle nhẹ thôi, không đợi count tăng vì app có thể không expose "Xóa nguyên liệu" ngay
        WaitingHelper.sleepSeconds(1);

        // Ping driver để tránh UiAutomator2 bị "đơ" sau khi add row
        try { getDriver().getPageSource(); } catch (Exception ignored) {}

        WaitingHelper.sleepSeconds(1);
    }

    @Step("Input ingredients (scroll từng ô + hide keyboard ngay)")
    public void inputIngredients(List<List<String>> ingredients) {

        // add đủ dòng (chị đang làm 2 lần)
        for (int i = 0; i < 2; i++) {
            slowSwipeDownOnScreen(1);
            click(btnAddIngredient);
            waitAfterAddIngredient();   // ✅ thêm đúng 1 dòng này
        }

        for (int i = 0; i < ingredients.size(); i++) {
            int row = i + 1;

            By field1 = ingredientField(row, 1);
            By field2 = ingredientField(row, 2);
            By field3 = ingredientField(row, 3);

            typeAndHideKeyboard(field1, ingredients.get(i).get(0)); // tên
            typeAndHideKeyboard(field2, ingredients.get(i).get(1)); // số lượng
            typeAndHideKeyboard(field3, ingredients.get(i).get(2)); // đơn vị
        }

        AllureHelper.attachScreenshot("Ingredients DONE");
    }

    @Step("Input steps (scroll từng ô + hide keyboard ngay)")
    public void inputSteps(List<String> steps) {
        ...
        for (int i = 0; i < steps.size(); i++) {
            int index = i + 1;
            By stepLocator = stepField(index);

            typeAndHideKeyboard(stepLocator, steps.get(i));
        }

        if (DEBUG_SCREENSHOT) AllureHelper.attachScreenshot("Steps DONE");
    }
    public void clickScreenAddRecipe(){
        click(tap3soc);
        click(btnDieuHuongAdd);
    }
//    public void ktra(){
//        click(ktra);
//    }
    ...
    //    public void clickPost() {click(btnPost);}
    @Step("Click Đăng công thức")
    public void clickPost() {
        // 1) scroll đến nút Đăng công thức
        ensureVisible(btnPost);

        // 2) tắt keyboard để không che nút
        try {
            getDriver().hideKeyboard();
        } catch (Exception ignored) {
        }

        // 3) click + fallback tap center
        try {
            WaitingHelper.waitForClickable(btnPost);
            click(btnPost);
        } catch (Exception e) {
            WebElement el = getDriver().findElement(btnPost);
...
//        click(gallery1); //dùng khi đổi tab
//        click(gallery2);
        if (index == 1) {
    ...
    public void openAddTagPopup() {
        ...
        if (DEBUG_SCREENSHOT) AllureHelper.attachScreenshot("Popup Thêm tag opened");
    }
    ...
    public String typeKeywordAndSelectDropdownTag(String keyword, String expectedSelectedText) {
        ...
        if (DEBUG_SCREENSHOT) AllureHelper.attachScreenshot("After typing tag");
    ...
    public void clickAddInPopup() {
        ...
        WaitingHelper.sleepSeconds(1);
        if (DEBUG_SCREENSHOT) AllureHelper.attachScreenshot("After click Add in popup");
    }
    ...
    public void verifyTagChipDisplayedInPopup(String tagName) {
        ...
        if (!found) {
            if (DEBUG_SCREENSHOT) AllureHelper.attachScreenshot("Tag chip NOT found in popup");
            throw new AssertionError("❌ Tag chip không hiển thị trong popup: " + tagName);
        }
        if (DEBUG_SCREENSHOT) AllureHelper.attachScreenshot("Tag chip found in popup");
    }
    ...
    public void closePopup() {
        ...
        click(btnClosePopup);
        WaitingHelper.sleepSeconds(1);
        if (DEBUG_SCREENSHOT) AllureHelper.attachScreenshot("Popup closed");    }
    ...
    public void verifyTagOutside(String tagName) {
        ...
        if (!isDisplayed(chip)) {
            if (DEBUG_SCREENSHOT) AllureHelper.attachScreenshot("Tag NOT found outside");
            throw new AssertionError("❌ Không thấy tag ngoài màn Edit: " + tagName);
        }
        if (DEBUG_SCREENSHOT) AllureHelper.attachScreenshot("Tag found outside");
    }
    ...
    public void addMultipleTags(List<String> tags) {
        ...
        // ✅ chỉ 1 ảnh sau khi add xong hết
        if (DEBUG_SCREENSHOT) AllureHelper.attachScreenshot("All tags added");
    }
}